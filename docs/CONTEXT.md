...
Four docks exist today: `hero` (the Hero's media slot), `about` (the About section's intro slot), and `tournamently`/`mathview` (each project's own "world," `mood: "observe"` — see §10's Projects entry). Adding a new resting place is still: mount a `<ScopeDock id="..." config={{...}} />` sized the way Scope should read there. Nothing else — SPR-005's `attentionTarget` (§4) is an addition to `ScopeDockConfig`, not a new registration mechanism.
...
- Only three sections exist in the live page (Hero + About + Projects) — there is no real Contact content yet.
...
- Project Detail and Contact experiences (Sprints 5 and 7 below). About (Sprint 6) and Projects (Sprint 4) are both done, Projects arriving after About — see above.
