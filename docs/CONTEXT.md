...
src/
  ...
  lib/email/resend.ts     The one abstraction boundary around the Resend
...
- The Contact section (SPR-009, "The Final Scene," redirected in SPR-009.1 — `src/components/sections/contact-section.tsx`) — the portfolio's closing chapter and Scope's first genuinely scripted moment: until now Scope has only ever rested at docks or glanced around; here it acts. The SPR-009.1 redirect's governing rule: every important transition must be attributable to Scope, not to a form or a bare timer — "if Scope disappeared from the scene, the interaction should stop making narrative sense." `contact-desk.tsx` (the section's client leaf) runs a five-scene state machine (`"arrival" | "preparing" | "writing" | "delivering" | "departed"`). Scenes 1–4 need no scripted Scope *motion*, but each transition is still framed as Scope's own doing: mounting `<ScopeDock id="contact">` alone produces arrival via the existing dock-follow system; his mood settling to `"observe"` visibly *precedes* the paper+pen appearing, by a held beat (`SETTLE_DELAY_S`), rather than both happening in the same frame; his own `useScopeAcknowledge()` reaction fires proactively right as the form is invited in, not in response to a visitor action; and his mood shifts to `"curious"` for as long as a submission is genuinely in flight (`ContactForm`'s `onSubmittingChange`, wired to a local `isSubmittingIdea` flag — previously plumbed but never consumed). Scope has no arms (see §4/`docs/scope-docs/scope/`), so nothing ever shows him manipulating the paper or pen directly — they react to his presence and decisions, timed against scene state, never the reverse. The form itself (`contact-form.tsx`, built on `@base-ui/react`'s `Form`/`Field` plus `Input`/`Textarea`/`Label` primitives in `src/components/ui/`) is deliberately quiet — borderless underline fields, small-caps labels, an outline-variant submit button, capped width — so the visual weight of the scene stays with Scope (rendered at dock `scale: 1`, Hero-level prominence, not the more modest 0.85 every other dock uses) rather than the form. Scenes 5–6 (delivery/departure) are the portfolio's second use of the `isSceneTransitioning`/`beginSceneTransition`/`getScopeMotionValues`/`sceneMood` commandeering mechanism theme-transition-controller.tsx introduced (see §6): Scope approaches the paper (`springs.layout`, a short local move — `springs.companion` stays reserved for cross-section travel), then *accepts* the idea using Scope's existing canonical `"happy"` mood (`scope-motion.ts`) — the one moment in the whole portfolio that mood's own doc comment reserves it for, "a reaction to the visitor's... work," never a celebration of itself, replacing an earlier bespoke "nod" gesture SPR-009.1 removed — holds for `SCOPE_HAPPY_HOLD_MS`, settles to a calm `"observe"` and holds again (`ACKNOWLEDGE_PAUSE_MS`) before departing at `"idle"`, so leaving reads as his own decision, not an animation simply ending. The paper folds/fades away under its own motion throughout (never Scope's, per the no-arms constraint). The exit target is measured against the desk panel's own current edges (`computeExitTarget`), not a fixed offset, since an early fixed-offset version read as "shrinks in place overlapping the closing line" once actually watched in the browser rather than "walks away" — and moving it far enough to clear the panel required adding `overflow-x-hidden` to both `<html>` and `<body>` in `layout.tsx`, since the transform can briefly exceed the document's normal content width. Deliberately never calls `endSceneTransition()` afterward: Contact is the last section, so Scope stays permanently commandeered/off-stage once it departs — required a new guard in `playThemeTransition` (fall back to a plain `setTheme()` if `isSceneTransitioning` is already true) so toggling theme afterward doesn't fight the departed state. The one line left on screen once Scope has left, replacing the whole panel: "Every great idea starts with a conversation." — confirmed exact wording with the site owner, deliberately not "Thank you"/"Message sent." Submission is real: `src/app/api/contact/route.ts` sends the visitor's message via Resend to the site owner's inbox (env var `RESEND_API_KEY`, documented in `.env.example`; from-address is Resend's shared `onboarding@resend.dev` sender pending a verified domain), with a dev-only `CONTACT_SIMULATE_SUCCESS` env-var bypass for exercising the full accept/departure sequence without a real API key, and a visually-hidden honeypot field (`contact-form.tsx`) for spam resistance with no visible CAPTCHA. No toast/modal/success UI anywhere — a successful delivery is represented entirely by Scope's own scripted reaction. SPR-009.2 hardened the backend into three layers, each with one job: `schemas/contact.ts` (Zod — server-side validation the client is never trusted to have already done, returning `422` on failure, distinct from a malformed-JSON `400` and a delivery-failure `500`), `lib/email/resend.ts` (the one place the `Resend` SDK is imported at all — exports `sendContactEmail()`, returning a plain `{ok: true} | {ok: false, reason}` the route handler never needs to interpret beyond that), and the route handler itself, now just: parse → honeypot check (on the raw body, *before* schema validation, so a filtered submission always gets an identical generic `200` regardless of whether its other fields would even have passed validation) → validate → `CONTACT_SIMULATE_SUCCESS` dev bypass (see `.env.example`) → `sendContactEmail()`. `ContactForm` and `contact-desk.tsx` needed zero changes for this — the cinematic experience and the delivery mechanism are fully decoupled, exactly as intended.
