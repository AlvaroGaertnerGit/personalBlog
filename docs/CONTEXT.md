...
- The Contact section (SPR-009, "The Final Scene" — redirected four times, in SPR-009.1 "Scope as protagonist," SPR-009.3 "the letter must feel real," SPR-009.4 "calmer, more physical, more believable," and SPR-009.5, a pure interaction-design pass on one beat — `src/components/sections/contact-section.tsx`) — the portfolio's closing chapter and Scope's first genuinely scripted moment: until now Scope has only ever rested at docks or glanced around; here he acts. Two governing rules carried through every redirect: (1) every important transition must be attributable to Scope — "if Scope disappeared from the scene, the interaction should stop making narrative sense" — and (2) physical continuity — the visitor is handing Scope a physical letter, an illusion that must never break, so the paper is one persistent element from the moment it appears to the moment it's accepted, never replaced or swapped for other content. SPR-009.4 added a third rule specifically because Scope has no arms (see `docs/scope-docs/scope/`): nothing here ever shows him manipulating the paper directly — it lifts, glides, and reshapes itself on its own timing, and he only ever watches and reacts.

  `contact-desk.tsx` (the section's client leaf) runs a four-scene state machine: `"ready" → "sealing" → "delivering" → "resting"` — collapsed from SPR-009.3's six scenes once the site owner's own review found the original "arrival"/"preparing"/"writing" setTimeout chain made the section wait ~2.3s before the visitor could even start typing. `"ready"` is the default from first render: desk, paper, clip, pen, and `<ContactForm>` are all already present and interactive — Scope's own travel into the section is the existing, always-unblocked continuous dock-follow system, so nothing here gates on it. A small `<Reveal>`-wrapped entrance (the same `whileInView`/`defaultViewport` convention used elsewhere on the page) gives the paper a tiny scroll-tied appearance, purely cosmetic — it never blocks interactivity.

  On a confirmed successful submission (`ContactForm`'s `onDelivered`, same trigger point as before — a real network failure still surfaces `errorMessage` and never plays a premature cinematic), the sequence runs with no fold anywhere: a held pause → the clip releases (unchanged from SPR-009.3) → the pen rolls a few degrees, permanently, "its purpose is finished" → the sheet lifts slightly off the surface (`paperY`/a small `paperScale` bump that fully resolves back to 1 before the next beat, never `scaleY`).

  SPR-009.5 redesigned only what happens next, after the site owner reviewed SPR-009.4 live and found the plain glide that followed — physically correct as it was — "predictable and slightly mechanical," the one weak beat left in an otherwise strong sequence. Chosen from three fundamentally different concepts designed and brutally self-critiqued before any code changed (an industrial "the desk itself tilts, gravity does the work" mechanism — rejected because a tilt subtle enough to stay premium doesn't actually explain the letter's motion under real physics, and one exaggerated enough to be legible reads as a surreal, unmotivated "living desk"; a museum-quiet "the paper's ambient motion stills under Scope's attention, then a hard cut" approach — rejected because it needs a persistent ambient flutter on the paper the entire time the visitor is writing, directly fighting the section's own immediate-usability requirement, and because "nothing visibly happening" risks failing silently against a web audience conditioned to expect visible feedback), the winning concept invents nothing new: `VISUAL_LANGUAGE.md` already names warm indirect light as Scope's "primary signal" and his one established way of touching the world ("extremely subtle atmospheric purple only when interacting with the environment"). The letter still travels — `paperX`/`paperY` still animate toward Scope's actual rendered position, read via `document.querySelector('[data-slot="companion-scope"]')` exactly as before, Scope still never moves for this beat — but only `GLIDE_DISTANCE_FRACTION` (0.7) of the full distance; the remaining gap is closed by light, not motion, via one new motion value, `arrivalGlow` (0→1), animated in the same `Promise.all` as the (shortened) lean-in rather than after it, so his light visibly reaches for the letter before it finishes arriving. `arrivalGlow` drives two things at once from one cause: an ambient warm bloom anchored toward Scope's side of the panel (a plain blurred `bg-scope-warm` circle, not precisely rect-measured — an ambient wash doesn't need spotlight precision), and a soft gradient wash directly on the letter itself, so it visibly starts "catching the light" rather than staying visually inert during its own approach. `envelopeProgress` (unchanged mechanic — narrower `paperScaleX`, a small flap's fade-in) now runs concurrently within this same lit window, so "catching the light" and "becoming an envelope" read as one transformation, never the same disappearing trick played twice before the actual accept. `arrivalGlow` recedes to 0 in the same beat as the final accept (paper shrinks/fades into Scope), so nothing lingers once the letter is gone.

  The accept-moment reaction reverted to the original `"happy"` mood hop (`scope-motion.ts`'s `SCOPE_HAPPY_ANIMATE`/`SCOPE_HAPPY_HOLD_MS`) rather than SPR-009.4's antenna pulse — the site owner explicitly preferred the original after living with both, so the `glowPulse` MotionValue SPR-009.4 had added to the shared `ScopeMotionValues` (`scope-dock-context.tsx`, `companion-scope.tsx`, and the antenna-tip circle in `scope.tsx`) was removed entirely rather than left as unused plumbing — a local `isCelebrating` boolean now drives `dockMood` to `"happy"` for the fixed hold time instead, the same contract every other caller of that mood already follows.

  Scope then does not leave forever. Once the pulse settles, `scene` flips to `"resting"`, which — as ordinary declarative rendering, not an imperative `animate()` sequence — both reveals the confirmation copy and shifts the `<ScopeDock id="contact">` wrapper's own className (`sm:order-2 sm:ml-auto sm:self-end`) to a corner of the same panel, alongside its `config` dropping to `scale: 0.85`/`facing: 12` (matching the scale convention every other dock already uses, and `facing`'s own documented purpose — "a static orientation bias" — reading as "turns back slightly"). `CompanionScope`'s existing positioning effect (in `companion-scope.tsx`, entirely unrelated to this file) picks up that config/position change on its own and travels there via the same heavy `springs.companion` spring that carries Scope between every other section — no bespoke exit animation needed. Because nothing here ever calls `beginSceneTransition()`/commandeers his position at all in this redesign (there was no beat where his `x`/`y`/`scale` needed to be driven directly — the paper moves, not him), his ordinary idle presence/personality (breathing, blinking, cursor tracking) never pauses even once, and scrolling away mid-"resting" and back re-syncs him exactly like any other dock, with no special-case recovery logic required — replacing SPR-009.3's shrink-to-near-invisibility-past-the-panel-edge exit, which stayed commandeered (and visibly frozen) until an explicit `endSceneTransition()` fired on scroll-away.

  The confirmation state (`CONTACT_ENDING_LINE` + `CONTACT_FORM_COPY.replayLabel`, "Write another letter") appears the moment the mission is accepted, inside the exact same permanent desk panel — the desk, its lighting, and the `<ScopeDock>` never unmount, only the letter itself ever leaves. Clicking replay remounts `<ContactForm key={resetCount}>` (a bumped counter) for a genuinely blank, fully interactive form with zero reset plumbing added to `contact-form.tsx` itself, resets every paper/pen/envelope motion value, and returns `scene` to `"ready"` — Scope's own travel back to his writing-time spot is, again, just the ordinary dock-follow system reacting to the config/position reverting, the same mechanism that carried him to the corner in the first place.
  ...
  SPR-010.1 fixed Scope actually appearing in a delivered email — a two-part bug, each part verified rather than assumed. First: `scope-static.tsx`'s output was originally a real inline `<svg>...</svg>` tag inside the email HTML; checked against caniemail.com (not guessed), that's unrendered by Gmail and Outlook outright (Apple Mail only partially) — the tag was silently stripped, not merely styled wrong, so nothing at that position ever painted. A `<img src>` reference to an actual `.svg` *file* fares much better per the same source (Gmail desktop, Apple Mail, even old Outlook desktop) but is explicitly broken on the Gmail mobile app for real Google accounts — this project's own recipient type — so a rasterized PNG remains the one option with no known-broken major client. `scope-static.tsx` was rewritten to use literal hex fills read from `SCOPE_EMAIL_THEME` instead of Tailwind classNames (Satori, below, has no Tailwind build to resolve those against) and is rasterized to a real PNG by `src/app/api/scope-mark/route.tsx` via `next/og`'s `ImageResponse` (Satori — already used by this project's `opengraph-image.tsx`; confirmed via its own docs that nested raw SVG elements are fully supported and converted to an embedded image internally). Both templates reference it as `SCOPE_MARK_URL` (`lib/email/shared.tsx`), an absolute URL built from `NEXT_PUBLIC_SITE_URL` — the same pattern `layout.tsx`/`sitemap.ts`/`robots.ts` already use. A first attempt at this fix (`scope-raster.tsx`, a second hand-authored copy of the same drawing with its own duplicated hex constants) was corrected back down to the one `scope-static.tsx` file once flagged — "there must only ever be one canonical Scope."

  Second, a real production bug distinct from the above: even with the PNG route itself working, the *URL* embedded in production email pointed at `https://alvarogaertner.com` (the bare apex domain), which Vercel's own domain settings 308-redirect to `https://www.alvarogaertner.com` — confirmed live via `curl -sI` against both hosts. Most email clients' image fetchers don't reliably follow a redirect for an embedded image, so this produced a genuine broken-image icon (a failed request), distinct from the earlier bug's silent invisibility (a stripped tag, no request at all). The fix is operational, not code: `NEXT_PUBLIC_SITE_URL` in Vercel must be set to whichever host actually returns `200` directly for that project's domain configuration.
